...
import { GoogleChartsService } from '../files-hash/google-charts.service';
import { FileHashEvent, HashWorkerMessage, Thread } from '../files-hash/hash-worker.types';
...
export class MultipleWorkerPoolComponent {
  ...
  public hashMultipleFiles(files: File[]): Observable<HashWorkerMessage> {
    ...
    return fromWorkerPool<Blob, HashWorkerMessage>(index => {
      const worker = new Worker('../files-hash/file-hash.worker', { name: `hash-worker-${index}`, type: 'module' });
